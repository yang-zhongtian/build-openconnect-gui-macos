...
mkdir -p ~/Downloads/build-openconnect-gui/rb-aarch64
brew install gnutls
cp /opt/homebrew/opt/gnutls/.brew/gnutls.rb ~/Downloads/build-openconnect-gui/rb-aarch64/
sed -I '' -e '/--with-default-trust-store-file/d' ~/Downloads/build-openconnect-gui/rb-aarch64/gnutls.rb
brew uninstall gnutls

# install modified gnutls formula
brew install --build-from-source --formula ~/Downloads/build-openconnect-gui/rb-aarch64/gnutls.rb gnutls

# adjust qt@6 homebrew formula to force target to macOS 12.0
mkdir -p ~/Downloads/build-openconnect-gui/rb-aarch64
brew install qt@6
cp /opt/homebrew/opt/qt/.brew/qt.rb ~/Downloads/build-openconnect-gui/rb-aarch64/
sed -I '' -e 's/-DCMAKE_OSX_DEPLOYMENT_TARGET=#{MacOS.version}.0/-DCMAKE_OSX_DEPLOYMENT_TARGET=12.0/' ~/Downloads/build-openconnect-gui/rb-aarch64/qt.rb
brew uninstall qt@6

# install modified qt@6 formula
brew install --build-from-source --formula ~/Downloads/build-openconnect-gui/rb-aarch64/qt.rb qt6
...
sed -I '' -e 's/CMAKE_OSX_DEPLOYMENT_TARGET "10.12"/CMAKE_OSX_DEPLOYMENT_TARGET "12.0"/' ~/Downloads/build-openconnect-gui/openconnect-gui-macos12-aarch64/CMakeLists.txt
